...
void setup_auxcr()
{
	...
	if (is_cpu_family(CPU_OMAP36XX)) {
		/* Can't execute SMI since could have been launched
		 * from LoLo where SMI is already done */
		__asm__ __volatile__("mrc p15, 0, %0, c1, c0, 1" : "=r"(i));
		/* Enable ASA and L1NEON */
		i |= 0x30;
		__asm__ __volatile__("mcr p15, 0, %0, c1, c0, 1" :: "r"(i));
	} else {
...
static int do_switch_ecc(cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
	...
	if (argc == 1) {
		...
		case NAND_ECC_4BIT_SOFT:
			printf("4bit BCH Software ECC\n");
			break;
		case NAND_ECC_8BIT_SOFT:
			printf("8bit BCH Software ECC\n");
			break;
		default:
			printf("Unknown ECC method %d\n", __FUNCTION__, nand->ecc.mode);
			return -1;
		}
		return 0;
	}
...
U_BOOT_CMD(
	...
	"[hw/sw] - Switch between NAND hardware/software (hw/sw) ecc algrithm\n"
	"[chip] - Internal to NAND chip (chip) ecc algorithm\n"
	"[bch4_sw] - 4-bit software (bch4_sw) software ecc algorithm\n"
	"[bch8_sw] - 8-bit software (bch4_sw) software ecc algorithm"
);
